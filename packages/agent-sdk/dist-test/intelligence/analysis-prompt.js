...
exports.buildAnalysisUserPrompt = exports.buildAnalysisSystemPrompt = void 0;
...
const describeBrand = (context) => {
    const brand = context.brandProfile;
    if (!brand)
        return '';
    const parts = [];
    if (brand.niche)
        parts.push(`Nicho: ${brand.niche}`);
    if (brand.brandVoice)
        parts.push(`Tom de voz: ${brand.brandVoice}`);
    if (brand.targetAudience)
        parts.push(`Público: ${brand.targetAudience}`);
    return parts.length ? `\n## Contexto da marca\n${parts.join('\n')}\n` : '';
};
/**
 ...
 */
const buildAnalysisSystemPrompt = (context, fields, enrichments) => {
...
exports.buildAnalysisSystemPrompt = buildAnalysisSystemPrompt;
const buildAnalysisUserPrompt = (userInput) => `Mensagem do usuário:\n"""\n${userInput}\n"""`;
