...
/** Cuts resolve_source may poll AssemblyAI STT for up to 40 minutes. */
const CUTS_EXECUTION_TIMEOUT_MS = 45 * 60 * 1000;

const resolveExecutionTimeoutMs = (agentId: string): number =>
  agentId === 'cuts' ? CUTS_EXECUTION_TIMEOUT_MS : EXECUTION_TIMEOUT_MS;
...
export async function executeRun(
  ...
  let executionFailureHandled = false;
  ...
  try {
    ...
    const executeStepLoop = async (): Promise<KernelRunResult> => {
    ...
    try {
      return await raceWithTimeout(
        executeStepLoop(),
        resolveExecutionTimeoutMs(agentDefinition.agentId),
        'Execution timed out',
      );
    } catch (raceError) {
      const raceMessage = raceError instanceof Error ? raceError.message : 'Unknown error';

      if (raceMessage === 'Execution timed out' && !executionFailureHandled) {
        executionFailureHandled = true;
        const errorMessage = 'Execution timed out';
