...
describe('assemblyAiModelSupportsTemperature', () => {
  it('omits temperature for Anthropic reasoning models', () => {
    ...
    expect(assemblyAiModelSupportsTemperature('anthropic/claude-sonnet-4-6')).toBe(false);
  });

  it('keeps temperature for other gateway models', () => {
    expect(assemblyAiModelSupportsTemperature('claude-sonnet-4-5-20250929')).toBe(true);
