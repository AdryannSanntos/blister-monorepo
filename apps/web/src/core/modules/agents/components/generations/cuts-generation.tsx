...
export const CutsGeneration = () => {
  ...
  const handleGenerate = async () => {
    ...
    addAgentRun({
      ...
      preview: t("runPreview", { count: nextCuts.length }),
      creditsUsed: 1.8,
    });
  };

  return (
    <div data-testid="cuts-page" className="flex flex-col gap-6">
      <FileDropzone
        onFileSelect={(file) => {
      ...
      {cuts.length > 0 ? (
        <>
          ...
          <Button
            variant="outline"
            onClick={() => router.push(getAgentHistoryPath("cuts"))}
          >
            {t("viewHistory")}
          </Button>
        </>
