...
export const ResearchGeneration = () => {
  ...
  const handleSubmit = form.handleSubmit(async (values) => {
    ...
    addAgentRun({
      ...
      preview: t("deliverable1"),
      creditsUsed: 0.4,
    });
  });

  return (
    <div data-testid="research-page" className="flex flex-col gap-6">
      <Form {...form}>
        ...
        <>
          <ul className="flex flex-col gap-2">
            {deliverables.map((item) => (
              <li
                key={item}
                className="rounded-[var(--r-lg)] border border-[var(--line-default)] bg-[var(--bg-sunken)] p-4"
              >
          ...
          <Button
            variant="outline"
            onClick={() => router.push(getAgentHistoryPath("research"))}
          >
            {t("viewHistory")}
          </Button>
        </>
