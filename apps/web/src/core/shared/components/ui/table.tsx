...
function TableFooter({ className, ...props }: React.ComponentProps<"tfoot">) {
  return (
    <tfoot
      ...
      className={cn(
        "border-t border-[var(--line-default)] bg-muted/50 font-medium [&>tr]:last:border-b-0",
        className,
      )}
      {...props}
    />
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "group border-b border-[var(--line-subtle)] transition-colors duration-[var(--dur-fast)] ease-[var(--ease-out)] hover:bg-[var(--bg-hover)] has-aria-expanded:bg-[var(--bg-hover)] data-[state=selected]:bg-[var(--bg-active)]",
        className,
      )}
      {...props}
    />
