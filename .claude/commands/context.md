...
- Profiles: BUSINESS (owner), ADMIN (platform)
- Termos UI: Campanha, Cérebro da Marca, Créditos, labels por agente (criar texto, gerar imagem, planejar…), Aprovar
- Zero jargão de IA (não usar "agente", "prompt", "LLM", "peça")
- Agentes **isolados** — sem pipeline automático; campanha = workspace; revisão **por agente**
...
- 1 company per user in MVP — access via `userType` + roles + `companyId`
- Agentes plugáveis e isolados (`POST /api/agents/:agentId/run`), RAG, créditos, revisão na AgentRun, auto-melhoramento por agentId
